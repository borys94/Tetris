...
export default class MenuState extends GameState<GameStateType> {
  update(): void {
    // No updates in menu
  }
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.font = '24px Arial'
    ctx.fillText('Press SPACE to start', ctx.canvas.width / ratio / 2, ctx.canvas.height / ratio / 2)
    ctx.fillText('Press ESC to exit', ctx.canvas.width / ratio / 2, ctx.canvas.height / ratio / 2 + 40)
  }
